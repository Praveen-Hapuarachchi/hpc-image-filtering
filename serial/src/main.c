...
int main(int argc, char **argv) {
    ...
    if (argc < 4) {
        printf("Usage: %s <input> <output_gaussian> <output_sobel>\n", argv[0]);
        return 1;
    }

    printf("Running Serial filter (1 thread)\n");
    ...
    double t0 = get_time();
    apply_gaussian_blur(imgIn, imgGaussian);
    double t1 = get_time();

    printf("Gaussian Blur  - Serial Execution Time : %.6f seconds\n", t1 - t0);

    /* Measure Sobel Edge Detection */
    double t2 = get_time();
    apply_sobel_edge_detection(imgIn, imgSobel);
    double t3 = get_time();

    printf("Sobel Edge Det - Serial Execution Time : %.6f seconds\n", t3 - t2);
